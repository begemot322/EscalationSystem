import { ESCALATION_API_URL } from '../constants.js';
...
export const createEscalation = async (escalationData) => {
    ...
    if (!response.ok) {
        ...
        throw new Error(errorData.title || "Не удалось создать эскалацию");
    }
...
export const getEscalationById = async (id) => {
    ...
    if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.title || "Эскалация не найдена");
    }
    return await response.json();
};
...
export const updateEscalation = async (id, escalationData) => {
    ...
    if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.title || "Не удалось обновить эскалацию");
    }
    return await response.json();
};